...
export default function Viewfaqs() {
    ...
    useEffect(()=>{
     getfaqs()
    },[searchQuestion,searchAnswer,searchOrder])
    ...
    let multiDelete=async ()=>{
      if(ids.length>=1){
       ...
       axios.post(`${apiBaseUrl}faq/multidelete`,obj)
       .then((res)=>res.data)
       .then((finalRes)=>{
        if (finalRes.status){
        ...
        showWarning("Please select at least one FAQ")
      }
    ...
    let changeStatus=()=>{
      if(ids.length>=1){
        ...
        axios.post(`${apiBaseUrl}faq/changestatus`, obj)
       .then((res)=>res.data)
       .then((finalRes)=>{
        if (finalRes.status){
        showSuccess(finalRes.message)
  ...
  return (
    <section className="w-full">
      <div className="border-b bg-white px-6 py-3 shadow-sm">
        <span className="text-gray-600">Home / faq / </span>
      ...
      >
        <div className="border-b border-slate-200 bg-slate-100 px-4 py-3">
          <h3 className="text-[20px] font-semibold text-slate-800">Search FAQ</h3>
        </div>
        <div className="grid gap-4 p-4 md:grid-cols-3">
          <div>
            <label className="mb-1 block text-sm font-medium text-gray-700">Question</label>
            <input type="text" name="q" value={searchQuestion} onChange={(e)=>setSearchQuestion(e.target.value)} className="block w-full rounded-lg border border-gray-300 px-3 py-2.5 text-sm text-gray-900 focus:border-blue-500 focus:ring-2 focus:ring-blue-100" placeholder="Search by question" />
          </div>
          <div>
            <label className="mb-1 block text-sm font-medium text-gray-700">Answer</label>
            <input type="text" name="answer" value={searchAnswer} onChange={(e)=>setSearchAnswer(e.target.value)} className="block w-full rounded-lg border border-gray-300 px-3 py-2.5 text-sm text-gray-900 focus:border-blue-500 focus:ring-2 focus:ring-blue-100" placeholder="Search by answer" />
          </div>
          <div>
      ...
      <div className="mx-auto min-h-[610px] max-w-[1220px] py-5">
        <div className="flex items-center justify-between rounded-t-md border border-slate-400 bg-slate-100 px-4 py-3">
          <div className="text-[26px] font-semibold">View faq</div>
        ...
        <div className="overflow-x-auto rounded-b-md border border-t-0 border-slate-400">
          <table className="min-w-[850px] w-full text-left">
            <thead className="bg-gray-50 text-xs font-semibold uppercase"><tr>
              ...
              <th className="p-4">S. No.</th><th className="p-4">Question</th>
              <th className="p-4">Answer</th><th className="p-4">Order</th>
            ...
            <tbody>
              {data.length >= 1 ? (
                data.map((obj, index) => {
                  return (
                    ...
                    >
                      ...
                      <td className="p-4">{obj.q}</td><td className="p-4">{obj.answer}</td>
                      ...
                      <td className="p-4 text-center"><Link to={`/faqs/edit/${obj._id}`} className="inline-flex">
                ...
                <tr><td colSpan="7" className="border-t p-6 text-center text-gray-500">No faq Found</td></tr>
              )}
            </tbody>
          </table>
        </div>
