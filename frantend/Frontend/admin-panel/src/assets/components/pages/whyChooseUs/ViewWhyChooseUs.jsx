...
export default function Viewwhychooseuss() {
  ...
  const getFieldValue = (obj, keys, defaultValue = "-") => {
    let value = keys.map((key) => obj?.[key]).find((item) => item !== undefined && item !== null && item !== "");
    return value ?? defaultValue;
  };

  let getwhychooseuss = useCallback(() => {
    let searchObj = {
      title: searchTitle,
      order: searchOrder,
    };
    ...
    axios
      .get(`${apiBaseUrl}whychooseus/view`, {
        params: searchObj,
      })
      .then((res) => res.data)
      .then((finalRes) => {
  ...
  let multiDelete = async () => {
    if (ids.length >= 1) {
      ...
      let obj = { ids };
      axios
        .post(`${apiBaseUrl}whychooseus/multidelete`, obj)
        .then((res) => res.data)
        .then((finalRes) => {
          if (finalRes.status) {
      ...
      showWarning("Please select at least one whychooseus");
    }
  };
  ...
  let changeStatus = () => {
    if (ids.length >= 1) {
      let obj = { ids };
      axios
        .post(`${apiBaseUrl}whychooseus/changestatus`, obj)
        .then((res) => res.data)
        .then((finalRes) => {
          if (finalRes.status) {
            showSuccess(finalRes.message);
  ...
  return (
    <section className="w-full">
      ...
      <div className="p-4">
        <div id="whychooseus-filter" className={`${showSearch ? "block" : "hidden"} relative my-3 w-full rounded-lg border border-slate-200 bg-white px-6 py-4 shadow-sm`}>
          <p className="py-2 text-[20px] font-semibold">Search Why Choose Us</p>
          ...
          <div className="mt-4 flex justify-end gap-3 border-t border-slate-200 pt-4">
            ...
            <button onClick={getwhychooseuss} className="inline-flex items-center gap-2 rounded-lg bg-indigo-600 px-6 py-2.5 text-white shadow-sm transition-all hover:bg-indigo-700"><FaMagnifyingGlass /> Search</button>
          </div>
        </div>
            ...
            <tbody>
              {data.length >= 1 ? data.map((obj, index) => (
                <tr key={obj._id} className="border-b bg-white">
                  ...
                  <td className="max-w-[220px] truncate px-2 py-4">{getFieldValue(obj, ["description", "desc", "Description"])}</td>
                  <td className="px-2 py-4">{getFieldValue(obj, ["rating", "ratings", "Rating"])}</td>
                  ...
                  <td className="px-2 py-4">
                    <Link to={`/why-choose-us/edit/${obj._id}`} className="inline-flex">
                ...
                <tr><td colSpan="9" className="border-t p-6 text-center text-gray-500">No Why Choose Us found</td></tr>
              )}
            </tbody>
          </table>
        </div>
