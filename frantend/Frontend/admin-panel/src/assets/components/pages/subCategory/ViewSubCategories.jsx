...
const apiBaseUrl=import.meta.env.VITE_APIBASEPATH
  ...
  let getsubCategory=useCallback(()=>{
    let searchObj={
      name:searchName,
      slug:searchSlug,
      order:searchOrder
  ...
  let multiDelete=async ()=>{
    if(ids.length>=1){
      ...
      let obj={ids}
      axios.post(`${apiBaseUrl}subcategory/multidelete`,obj)
      .then((res)=>res.data)
      .then((finalRes)=>{
        if(finalRes.status){
  ...
  let changeStatus=()=>{
    if(ids.length>=1){
      let obj={ids}
      axios.post(`${apiBaseUrl}subcategory/changestatus`,obj)
      .then((res)=>res.data)
      .then((finalRes)=>{
        if(finalRes.status){
          showSuccess(finalRes.message)
  ...
  return (
    <section className='w-full'>
      ...
      <div className='p-4'>
        <div id='sub-category-filter' className={`${showSearch ? 'block' : 'hidden'} py-4 relative px-6 my-3 rounded-lg border border-slate-200 w-full bg-white shadow-sm`}>
          <p className='font-semibold py-2 text-[20px]'>Search Sub Category</p>
          ...
          <div className='flex justify-end gap-3 border-t border-slate-200 mt-4 pt-4'>
            ...
            <button onClick={getsubCategory} className='inline-flex items-center gap-2 text-white bg-indigo-600 hover:bg-indigo-700 px-6 py-2.5 rounded-lg shadow-sm transition-all'><FaMagnifyingGlass /> Search</button>
          </div>
        </div>
        ...
        <div className='border border-t-0 rounded-b-md border-slate-300 overflow-x-auto'>
          <table className='min-w-[900px] w-full text-center text-gray-700'>
            <thead className='text-sm uppercase bg-gray-50 border-b'><tr>
              ...
              <td className='px-2 py-4'><input type='checkbox' value={Obj._id} checked={ids.includes(Obj._id)} onChange={getCheckValue} className='sub-category-row-check w-4 h-4 cursor-pointer' /></td>
              ...
              <td className='px-2 py-4'>
                <Link to={`/sub-category/edit/${Obj._id}`} className='inline-flex'>
