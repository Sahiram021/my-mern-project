...
export default function Viewcountrys() {
    ...
    let multiDelete=async ()=>{
      if(ids.length>=1){
       ...
       axios.post(`${apiBaseUrl}country/multidelete`,obj)
       .then((res)=>res.data)
       .then((finalRes)=>{
        if (finalRes.status){
        ...
        showWarning("Please select at least one country")
      }
    ...
    let changeStatus=()=>{
      if(ids.length>=1){
        ...
        axios.post(`${apiBaseUrl}country/changestatus`, obj)
       .then((res)=>res.data)
       .then((finalRes)=>{
        if (finalRes.status){
        showSuccess(finalRes.message)
  ...
  return (
    <section className="w-full">
      <div className="border-b bg-white px-6 py-3 shadow-sm">
        <span className="text-gray-600">Home / country / </span>
      ...
      >
        <div className="border-b border-slate-200 bg-slate-100 px-4 py-3">
          <h3 className="text-[20px] font-semibold text-slate-800">Search Country</h3>
        </div>
      ...
      <div className="mx-auto min-h-[610px] max-w-[1220px] py-5">
        <div className="flex items-center justify-between rounded-t-md border border-slate-400 bg-slate-100 px-4 py-3">
          <div className="text-[26px] font-semibold">View country</div>
          ...
          <table className="min-w-[850px] w-full text-left">
            ...
            <tbody>
              {data.length >= 1 ? (
                data.map((obj, index) => {
                  return (
                    ...
                    >
                      ...
                      <td className="p-4 text-center"><Link to={`/country/edit/${obj._id}`} className="inline-flex">
                ...
                <tr><td colSpan="6" className="border-t p-6 text-center text-gray-500">No country Found</td></tr>
              )}
            </tbody>
          </table>
        </div>
